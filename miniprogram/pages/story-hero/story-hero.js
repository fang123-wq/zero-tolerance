...
Page({
  ...
  onResultVideoEnded: function() {
    ...
    storyReward.claimStoryEndingReward(
      app,
      'story_hero',
      that.data.chosenOption || 'unknown',
      '沉浸式故事完成：英雄篇-' + (that.data.chosenOption || 'unknown')
  ...
  startChoiceCountdown: function() {
    var that = this;
    var seconds = 15;
    this.clearChoiceTimer();
    this.setData({ countdown: seconds });
    this.choiceTimer = setInterval(function() {
      var next = that.data.countdown - 1;
      if (next <= 0) {
        that.clearChoiceTimer();
        var randomOption = Math.random() < 0.5 ? 'mission' : 'family';
        that.chooseOption({ currentTarget: { dataset: { option: randomOption } } });
      } else {
  ...
  onShareAppMessage: function() {
    return {
      title: '\u7f09\u6bd2\u82f1\u96c4\u7684\u6295\u62e9',
      path: '/pages/story-hero/story-hero'
    };
  }
});
