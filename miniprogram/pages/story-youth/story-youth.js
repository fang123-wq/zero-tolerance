var storyData = require('../../data/storyYouth.js').storyYouthData;
...
Page({
  ...
  loadStoryMusics: function() {
    var that = this;
    api.storyMusicApi.getByStory('story_youth').then(function(musics) {
  ...
  handleEndingComplete: function(scene) {
    var that = this;
    storage.markStoryCompleted('story_youth_' + scene.id);
  ...
  onShareAppMessage: function() {
    var scene = this.data.currentScene;
    var title = '我在《选择的岔路》少年篇中获得了"' + (scene.title || '未知') + '"结局！';

    return {
      title: title,
      path: '/pages/story-youth/story-youth',
